...
bool Map::is_solid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    int tile_x = static_cast<int>(floor((position.x + (m_tile_size / 2.0)) / m_tile_size));
    int tile_y = static_cast<int>(-ceil((position.y - (m_tile_size / 2.0)) / m_tile_size)); // Array counts up as Y goes down.
